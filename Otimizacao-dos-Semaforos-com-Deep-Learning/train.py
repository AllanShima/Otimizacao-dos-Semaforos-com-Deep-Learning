...
def phaseDuration(junction, phase_time, phase_state):
    ...
    # Define positions for each semaphore
    sem_1 = [0, 1, 2]  # 0-based index positions
    sem_2 = [3, 4, 5]
    sem_3 = [6, 7, 8]
    sem_4 = [9, 10, 11]
    tofind = "GGG"  # Letters you expect to find at these positions

    # Function to check if the substring tofind is present at any of the positions
    def find_text_at_positions(phase_state, positions, tofind):
        length = len(tofind)
        for pos in positions:
            if phase_state[pos:pos+length] == tofind:
                return True
        return False

    # Check each sem_x list
    if find_text_at_positions(phase_state, sem_1, tofind):
        print("Found in sem_1")
        command = "sem1"
        serialInst.write(command.encode('utf-8'))

    if find_text_at_positions(phase_state, sem_2, tofind):
        print("Found in sem_2")
        command = "sem2"
        serialInst.write(command.encode('utf-8'))

    if find_text_at_positions(phase_state, sem_3, tofind):
        print("Found in sem_3")
        command = "sem3"
        serialInst.write(command.encode('utf-8'))

    if find_text_at_positions(phase_state, sem_4, tofind):
        print("Found in sem_4")
        command = "sem4"
        serialInst.write(command.encode('utf-8'))
...
def run(train=True,model_name="model",epochs=50,steps=500,ard=False):
    ...
    if train:
        ...
        plt.savefig(f'plots/tempo_vs_epoch_{model_name}.png')
        plt.show()
